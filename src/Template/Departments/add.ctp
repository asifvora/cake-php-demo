...
<script>
    function Add_More ()
    {
        ...
        if ( start <= maxLimit )
        {
            $ ( ".add_div" ).append ( '<div id="add_more" ><div class="form-group "><label for="" class="control-label col-xs-4"><h5><b>Name :</b></h5></label><div class="input-group input-group m-t-10"><?php echo $this->Form->control(' ', array('type'=>'text', 'name'=>'name[][name]', 'required'=>'required', 'oninvalid'=>'setCustomValidity("Enter Department Name")', 'oninput'=>'setCustomValidity("")', 'class'=>'form-control '));?><span class="input-group-btn"><button type="button" class="btn btn-danger" title="Remove" onclick="Remove_More() ;"><i class="fa fa-minus"></i></button></span><br></div></div></div>' ) ;
            start++ ;
        }
    }
